 ...
 */
export function generateInstanceId(type, suffix) {
 ...
 */
export function isInstanceActive(instance, timeoutMs = 60000) {
    const now = Date.now();
 ...
 */
export function findAvailableInstances(instances, taskType, excludeIds = []) {
    const available = [];
 ...
 */
function getRequiredCapability(taskType) {
    switch (taskType) {
        case 'develop':
            return 'development';
        case 'review':
 ...
 */
export function checkResourceConflicts(resources, resourceType, resourceId, operation, requestingInstanceId) {
    const conflicts = [];
 ...
 */
function hasOperationConflict(existingOperation, newOperation) {
 ...
 */
export function cleanupExpiredResources(resources) {
    const now = new Date();
    let cleaned = 0;
    for (const [key, claim] of resources) {
 ...
 */
export function createResourceKey(resourceType, resourceId, instanceId) {
...
export function validateMethodParams(method, params) {
    ...
    switch (method) {
        case 'selfie.register':
            return validateRegisterParams(params);
        case 'selfie.heartbeat':
            return validateHeartbeatParams(params);
        case 'selfie.assign_task':
            return validateAssignTaskParams(params);
        default:
...
function validateRegisterParams(params) {
    ...
    if (!inst.type || !['initializer', 'developer', 'reviewer', 'tester'].includes(inst.type)) {
